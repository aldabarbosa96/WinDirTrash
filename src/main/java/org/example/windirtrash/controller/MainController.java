...
public class MainController {
    ...
    @FXML private TreemapPane treemapPane;
    @FXML private ChoiceBox<File> driveChoice;
    @FXML private Button scanBtn;
    @FXML private HBox driveBar;
    @FXML private HBox topBar;
    @FXML private Button autoCleanBtn;
    ...
    private void initialize() {
        ...
        driveChoice.setConverter(new StringConverter<>() {
            @Override public String toString(File f) {
            ...
            @Override public File fromString(String s) { return null; }
        });
        ...
        treeView.setCellFactory(tv -> {
            ...
            return new TreeCell<>() {
                @Override protected void updateItem(String item, boolean empty) {
                    ...
                    if (empty || item == null) {
                        setText(null); setGraphic(null); setTooltip(null); setStyle("");
                        return;
                    }
                    setText(item);
    ...
    private void onEscanear() {
        ...
        if (selDrive.toPath().getParent()==null) {
            var w = new Alert(Alert.AlertType.CONFIRMATION,
                    "Escanear " + selDrive.getPath() + " completo puede tardar.\n¿Continuar?");
        ...
        treemapPane.setHighlightCategory(null);
        treeView.setRoot(new TreeItem<>("Basura encontrada") {{ setExpanded(true); }});
        ...
        scanTask.setOnSucceeded(ev -> {
            ...
            paint.setOnSucceeded(e2 -> {
                ...
                boolean hasSafe = lastJunk.stream()
                        .anyMatch(fn -> CategoryInfo.get(fn.getCategory()).risk()==CategoryInfo.Risk.SAFE);
    ...
    private void onBorrarSeguro() {
        ...
        List<FileNode> safeNodes = lastJunk.stream()
                .filter(fn -> CategoryInfo.get(fn.getCategory()).risk() == CategoryInfo.Risk.SAFE)
        ...
        conf.setContentText(String.format(
                "Se eliminarán %d elementos seguros\n– liberando hasta %s de espacio.\n¿Continuar?",
                paths.size(), humanTotal
        ));
    ...
    private void runAutoDeleteTask(List<Path> tgt) {
        ...
        task.setOnSucceeded(ev -> {
            ...
            statusLabel.setText(String.format(
                    "Auto‑limpieza completada: %d elementos eliminados; %s liberados.",
                    actualCount, humanFreed
            ));
            autoCleanBtn.setVisible(false);
            autoCleanBtn.setManaged(false);
            onEscanear();
    ...
    private void addJunkToUI(FileNode n) {
        ...
        long total = catItem.getChildren().stream()
                .map(TreeItem::getValue)
                .mapToLong(this::bytesFromDisplay)
                .sum();
    ...
    private void deleteSelected(boolean wholeCat) {
        ...
        String cat = sel.isLeaf()
                ? sel.getParent().getValue().split(" \\(")[0]
                : sel.getValue().split(" \\(")[0];
